...
def text(rel: str) -> str:
    p = ROOT / rel
    if not p.exists():
        raise SystemExit(f"V4.11 VALIDATION FAILED: missing {rel}")
    return p.read_text(encoding="utf-8")

def must(cond: bool, msg: str) -> None:
    if not cond:
        raise SystemExit(f"V4.11 VALIDATION FAILED: {msg}")
...
must('"release": "v4.11-shanghai-local-intelligence"' in main, "build release not V4.11")
...
print("V4.11 SHANGHAI LOCAL INTELLIGENCE VALIDATION PASSED")
print("03:00 Shanghai municipal -> 16 districts -> street/community collector; Sanle -> Jing'an -> Jiangning Road -> Sanleli; national generic automatic pool retired; AI remains serial")
